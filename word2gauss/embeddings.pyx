...
cdef class GaussianEmbedding:
    ...
    cdef float train_batch(self, uint32_t k, np.ndarray[uint32_t, ndim=2, mode='c'] pairs):
        ...
        cdef float x
        with nogil:
            return train_batch(&pairs[0, 0], k, pairs.shape[0],
...
cdef float train_batch(
        uint32_t*pairs, uint32_t thread, size_t Npairs,
...
cdef void _accumulate_update(
    ...
    for i in range(K):
        ...
        mu_up = fac * local_eta * dmu[i]
        print "mu = %s mu grad = %s" % (mu_ptr[k*K+i], mu_up)
    ...
    if covariance_type == SPHERICAL:
        ...
        for i in range(K):
            ...
            print "sigma = %s sigma grad = %s" %(sigma_ptr[k*K+i], sigma_up)
